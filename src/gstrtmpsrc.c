...
gst_rtmp_src_class_init (GstRTMPSrcClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_LIVE,
      g_param_spec_boolean ("live", "to  allow read stream rtmp", 
      "to  allow read stream rtmp", FALSE, G_PARAM_READWRITE));
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_LIVE:
      src->live = g_value_get_boolean (value);
      break;
    default:
    ...
    GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    case PROP_LOCATION:
      ...
      break;
    case PROP_LIVE:
      g_value_set_boolean (value, src->live);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
